...
SAVE_FOLDER = f"{CORRELATIONS_FOLDER}/img"
...
ADAPTATION_METHODS = ["finetuning", "linear_probing", "bitfit"]
METRICS = ["DP", "EO (y=0)", "EO (y=1)", "OOD"]
...
ADAPTATION_COLORS = ["b", "r", "g"]
METRIC_COLORS = ["c", "y", "m", "b"]
MIN_SEED = 1
MAX_SEED = 5
...
for train_dataset in TRAIN_DATASETS:
    ...
    for metric in METRICS:     
        ...
        for adaptation_method, adaptation_color in zip(ADAPTATION_METHODS, ADAPTATION_COLORS):
            ...
            plt.plot(df_curr['checkpoint_step'], df_curr[metric], color=adaptation_color, label=f"{adaptation_method} {seed}")
        plt.legend(bbox_to_anchor=(1.05, 1))
    ...
    for adaptation_method in ADAPTATION_METHODS:
        ...
        across_seeds_df = across_seeds_df.reset_index()
        for metric, metric_color in zip(METRICS, METRIC_COLORS):
            plt.plot(across_seeds_df['checkpoint_step'], across_seeds_df[f"{metric} norm"], color=metric_color, label=f"{metric}")
        plt.legend(bbox_to_anchor=(1.05, 1))
