...
# temporary fix
        ...
        return self.model, TrainOutput(self.global_step, tr_loss.item() / self.global_step, {"loss": best_eval_loss, "acc": best_eval_acc})
        # not sure if the last argument is correctly formed
    ...
    def prediction_loop(
                ...
                if self.task in ["autoencoder", "mlm"]:
                    ...
                    if preds is not None and preds.size(1) != logits.size(1):
                        if preds.size(1) > logits.size(1):
                            ...
                            preds = temp
                preds = logits if preds is None else nested_concat(preds, logits, padding_index=0)
                ...
                if self.task in ["autoencoder", "mlm"]:
                    if label_ids is not None and label_ids.size(1) != labels.size(1):
                        if label_ids.size(1) > labels.size(1):
                            ...
                            label_ids = temp
                label_ids = labels if label_ids is None else nested_concat(label_ids, labels, padding_index=0)
        ...
        if self.task not in ["autoencoder", "mlm"]:
            if preds is not None:
...
#                 metrics["eval_loss"] = np.mean(eval_losses)
                metrics["eval_loss"] = torch.mean(torch.stack(eval_losses))
    ...
    def evaluate(self, eval_dataset: Optional[Dataset] = None) -> Dict[str, float]:
        ...
        print(f"Output metrics for {eval_dataset}: {output.metrics}")
