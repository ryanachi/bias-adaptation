...
def get_word2positions(ids, tokenizer, model_name):
    ...
    if 'bert-' in model_name:
        ...
        while i < n-1:
            ...
            start, end = i, i + 1 
            i += 1
            while (i < n-1 and not(tokens[i].startswith(delimiter))):
                wordpiece = tokens[i]
                if wordpiece not in {',', '.', '!', ':', ';', '?'}:
                    word += tokens[i]
                    end += 1
...
def fetch_canonical_embeddings(sequences, layers, model_name, model_path, file_descriptor, is_model_adapted=False, device='cpu', sentences_per_context=3, pickled=False):
    if is_model_adapted == False:
        model_path = model_name # load from HF website
    
    N, V = sentences_per_context, len(sequences)
    file_name = cwd / "data" / "contextualized" / f"{file_descriptor}_{model_path}_{N}_context_{layers}_layers_{V}_vocab.pkl"
    if pickled:
        representations, word2count, missing_words = pickle.load(open(file_name, 'rb'))
    else:
        model, tokenizer = AutoModel.from_pretrained(model_path), AutoTokenizer.from_pretrained(model_name, use_fast = True)
#         model.to(device)
#         representations = {word : None for word in vocab}


        # "sequences" --> str        : List[List[List[str]]]
        #                 vocab_word : [sequence[sentence[word]]]
        # so the vocab words === `sequences.keys()`
        
        representations = {word : {layer : None for layer in range(layers+1)} for word in sequences.keys()}

        word2count = {word : 0 for word in sequences.keys()} # Number of contexts/sequences a word appears in
        missing_words = set()
        documents = []
        
        for vocab_word, adjacent_sentences_list in tqdm(sequences.items()):
            sequences_list = []
            for adjacent_sentence_sequence in adjacent_sentences_list:
                sequence = ""
                if sentences_per_context == 1:
                    sequence = " ".join(adjacent_sentence_sequence[0])
                else:
                    sequence_words_broken_up = []
                    for sent in adjacent_sentence_sequence:
                        sequence_words_broken_up.extend(sent)
                    sequence = " ".join(sequence_words_broken_up) # results in " , " and " . " -- perhaps fix later?
                
            
                ids_tensor, ids = tokenizer(sequence, padding='max_length', truncation=True, return_tensors = 'pt')['input_ids'], tokenizer(sequence, padding='max_length', truncation=True)['input_ids']
                word2positions = get_word2positions(ids, tokenizer, model_name)
                
                if vocab_word in word2positions:
                    for layer in range(layers+1):
                        ...
                        for word, positions in word2positions.items():
                            if word == vocab_word:
                                ...
                                if representations[word][layer] is None:
                                    representations[word][layer] = representation
                                else:
                                    representations[word][layer] += representation 
        for word, representation in representations.items():
            if representation is None:
                ...
                for layer in range(layers+1):
                    ...
                    representations[word][layer] = decontextualized_representation.detach().numpy()
