...
cwd = Path("/u/scr/nlp/mercury/bias-adaptation")
...
def make_contexts(documents, context_length, resource_name, n=False, pickled=False):
    ...
    if n:
        file_name = cwd / "datasets" / "contexts" / "{}_{}contexts-len={}.pkl".format(resource_name, n, context_length)
    else:
        file_name = cwd / "datasets" / "contexts" / "{}_contexts-len={}.pkl".format(resource_name, context_length)
    if pickled:
